 ...
 * creating rooms, the player and dropper, as well as updating information when
...
public class GameEngine {
     ...
     * items have been collected to allow the player to exit via a DOOR tile and
     ...
     * screen by the GUI class, and by you to check what a specific Tile
     * contains by checking the content of specific elements of the array and
     ...
     * and update the position of the dropper.
     */
    ...
    /**
     * Generates a new level. This method should instantiate the room array, and
     ...
     * assigning the Tile object to an element in the array. For the first
     * version you should use just Tile objects with the type BACKGROUND.
     *
     * Later tasks will require additions to this method to add new content, see
     * the specification document for more details.
     */
    private void generateRoom(int doorX, int doorY) {
        this.room = new Tile[35][18];
        ...
        for (int y = 0; y < 18; y++) {
            for (int x = 0; x < 35; x++) {
                if (x == 0 | x == 34) {
                    ...
                    room[x][y] = new Tile(TileType.CEILING);
                } else if (y == 17) {
                    room[x][y] = new Tile(TileType.FLOOR);
                } else {
    ...
    private void addDropper() {
        this.dropper = new Dropper(18, 1);

        int currLevel = this.levelNumber;
        int range = room.length;

        int drpX = dropper.getX() * range - 1;
        int drpY = dropper.getY();

        if (currLevel >= 1) {
            drpX = rng.nextInt(range);
        }

        dropper.setPosition(drpX, drpY);
     ...
     * class.
     *
     * The player should be placed at the bottom of the level by using
     * appropriate X and Y co-ordinate values when calling the constructor. The
     * energy value can be set to any integer for your first version of this
     * method, but may need updating in a later task.
     */
    private void createPlayer() {
        this.player = new Player(100, 1, 16);
     ...
     * (use the relevant methods from the Entity/Dropper class to get the
     * current position of the dropper and calculate the co-ordinates of the
     * space below it).
     */
    private void activateDropper() {

        int drpY = dropper.getY(); // get dropper x position 
        int drpX = dropper.getX(); // get dropper y position 

        if (room[drpX][drpY + 1].getType() == TileType.BACKGROUND) {

            int Chance = rng.nextInt(50); // get random number from range of 50

            TileType type;
            if (Chance < 37.5) {
                type = TileType.BOX;
            } else {
                type = TileType.ITEMBOX;
            }

            room[drpX][drpY + 1] = new Tile(type);
        }
     ...
     * the user has pressed the left arrow key on the keyboard. Your code should
     * set the X and Y position of the player to place them in the correct tile
     * based on the direction of movement, i.e. changing their X position by
     * one. You will need to use the relevant method(s) from the Player/Entity
     * class to find the current position of the player, and set new values
    ...
    public void movePlayerLeft() {
        ...
        int plyrX = player.getX(); // get player x position 
        int plyrY = player.getY(); // get player y position

        if (plyrX > RoomLimit) {
            plyrX--;
            player.setPosition(plyrX, plyrY);
        }

        //player climbing
        if (room[plyrX][plyrY].getType() == TileType.BOX && room[plyrX][plyrY - 1].getType() == TileType.BACKGROUND) {
            player.setPosition(plyrX, plyrY - 1);
        } else if (room[plyrX][plyrY].getType() == TileType.BOX && room[plyrX][plyrY - 1].getType() != TileType.BACKGROUND) {
            player.setPosition(plyrX + 1, plyrY);
        }

        if (room[plyrX][plyrY].getType() == TileType.ITEMBOX && room[plyrX][plyrY - 1].getType() == TileType.BACKGROUND) {
            player.setPosition(plyrX, plyrY - 1);
        } else if (room[plyrX][plyrY].getType() == TileType.ITEMBOX && room[plyrX][plyrY - 1].getType() != TileType.BACKGROUND) {
            player.setPosition(plyrX + 1, plyrY);
        }

        if (room[plyrX][plyrY].getType() == TileType.ITEM) {
            room[plyrX][plyrY] = new Tile(TileType.BACKGROUND);
            itemsCollected++;
        }
    }
     ...
     * Handles the movement of the player when attempting to move in the game.
     * This method is automatically called by the GameInputHandler class when
     * the user has pressed the right arrow key on the keyboard. Your code
     * should set the X and Y position of the player to place them in the
     * correct tile based on the direction of movement, i.e. changing their X
     * position by one. You will need to use the relevant method(s) from the
     * Player/Entity class to find the current position of the player, and set
     * new values based on the current X and Y values.
     *
     * In later tasks you will need to update this method to handle more complex
     * movement cases (e.g. only moving into BACKGROUND tiles, collecting items
     * and climbing boxes).
    ...
    public void movePlayerRight() {
        ...
        if (room[plyrX][plyrY].getType() == TileType.ITEMBOX && room[plyrX][plyrY - 1].getType() == TileType.BACKGROUND) {
            player.setPosition(plyrX, plyrY - 1);
        } else if (room[plyrX][plyrY].getType() == TileType.ITEMBOX || room[plyrX][plyrY].getType() == TileType.BOX && room[plyrX][plyrY - 1].getType() != TileType.BACKGROUND) {
            player.setPosition(plyrX - 1, plyrY);
        }
        ...
        if (room[plyrX + 1][plyrY].getType() == TileType.DOOR) {
            player.setPosition(plyrX + 1, plyrY);
        }
    }
    ...
    public void breakBoxes() {
        ...
        int plyrEng = player.getEnergy();

        if (plyrEng > 0) {

            if (room[plyrX + 1][plyrY].getType() == TileType.BOX) {
                room[plyrX + 1][plyrY] = new Tile(TileType.BACKGROUND);
                player.changeEnergy(-25);
            } else if (room[plyrX - 1][plyrY].getType() == TileType.BOX) {
                room[plyrX - 1][plyrY] = new Tile(TileType.BACKGROUND);
                player.changeEnergy(-25);
            }
            ...
            if (room[plyrX + 1][plyrY].getType() == TileType.ITEMBOX) {
                room[plyrX + 1][plyrY] = new Tile(TileType.ITEM);
                player.changeEnergy(-25);
            } else if (room[plyrX - 1][plyrY].getType() == TileType.ITEMBOX) {
                room[plyrX - 1][plyrY] = new Tile(TileType.ITEM);
                player.changeEnergy(-25);
            }
     ...
     *
     * A later version of this method will require you to move the dropper by
     * checking the player object's position and "following" the player.
     */
    private void moveDropper() {

        int drpX = dropper.getX(); // get dropper x position
        int drpY = dropper.getY(); // get dropper y position

        int targetPlyr = player.getX(); // get player x position
    ...
    /**
     * Applies gravity to the player object. This method should check if the
     * Tile below the player is of the type BACKGROUND; if it is then the
     * player's position should be changed to "drop" the player one Tile into
     * the empty Tile below their current position. You will need to retrieve
     * their current position using the appropriate methods in the Player/Entity
     * class.
     */
    private void applyPlayerGravity() {
        ...
        if (room[plyrX][plyrY + 1].getType() == TileType.BACKGROUND) {
            plyrY++; //player gravity 
            player.setPosition(plyrX, plyrY);
        }
     ...
     * the BOX is of the type BACKGROUND. If the tile below the BOX is a
     * BACKGROUND tile then the BOX should be "dropped" one tile by creating new
     * objects and placing them into the room array at the correct positions, or
     * swapping the BOX and BACKGROUND tile objects in the array. After
     * dropping, the BOX should now be one tile lower and the position it used
     * to be in should be a BACKGROUND tile.
     */
    private void applyBoxGravity() {
        ...
        for (int colX = 0; colX < room.length; colX++) {
            for (int rowY = room[colX].length - 1; rowY >= 0; rowY--) {

                if (room[colX][rowY].getType() == TileType.BOX) {
                    if (room[colX][rowY + 1].getType() == TileType.BACKGROUND) {

                        room[colX][rowY] = new Tile(TileType.BACKGROUND);
                        room[colX][rowY + 1] = new Tile(TileType.BOX);

                    }
                    if (room[plyrX][plyrY].getType() == TileType.BOX) {
                        startGame();
                        levelNumber = 1;
                        itemsCollected = 0;
                    }
                ...
                if (room[colX][rowY].getType() == TileType.ITEMBOX) {
                    if (room[colX][rowY + 1].getType() == TileType.BACKGROUND) {

                        room[colX][rowY] = new Tile(TileType.BACKGROUND);
                        room[colX][rowY + 1] = new Tile(TileType.ITEMBOX);
                    }
                }

                if (room[colX][rowY].getType() == TileType.BOX) {
                    if (room[colX][rowY + 1].getType() == TileType.ITEM) {
    ...
    private boolean levelComplete() {
        ...
        if (room[plyrX][plyrY].getType() == TileType.DOOR && itemsCollected >= levelNumber + 2) {
            return true;
        }
    ...
    /**
     * This method is called when the player completes the current level by
     * reaching a DOOR tile, optionally also requiring some ITEM tiles to be
     * collected first (in a later task).
     *
    ...
    private void nextLevel() {
        ...
        startGame();
        placePlayer(1, 16);
        generateRoom(34, 16 - levelNumber + 1);
    ...
    public void doTurn() {
        int plyrEng = player.getEnergy();
        int maxEng = player.getMaxEnergy();
        ...
        if (turnNumber % 8 == 0 && plyrEng < maxEng) {
            player.refillEnergy();
        }
        if (levelComplete()) {
            nextLevel();
